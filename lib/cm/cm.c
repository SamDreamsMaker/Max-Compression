...
static uint16_t cm_predict(cm_t *cm, uint32_t pos, int bp, float *str, uint16_t *raw_mp) {
    ...
    int mx6_ctx = ((cm->line_pos < 8 ? 1 : 0) << 7) | (mlen_bucket << 5) | ((cm->prev[0] >> 6) << 3) | bp;
    ...
    float s4 = stretch(m4); float s1 = stretch(m1), s8 = stretch(m8); float s6 = stretch(m6); float s3 = stretch(m3); float s7 = stretch(m7); float s2 = stretch(m2);
    float cross = s1 * s8 * 0.006f + s3 * s8 * 0.004f + s2 * s6 * 0.003f;
    ...
    int apm2_ctx = ((cm->prev[0] >> 4) << 8 | (cm->partial & 0x1F) << 3 | bp) & (SSE_CTXS-1);
    ...
    uint16_t final = (apm2_p * 1 + apm3_p * 2 + par_p * 3 + mp * 56) / 62;
...
static void cm_update(cm_t *cm, uint32_t pos, int bp, int bit,
    ...
    mixer_learn(&cm->mx3[bp], str, bit, lr * 0.7f);
    {
        ...
        int mx6_ctx = ((cm->line_pos < 8 ? 1 : 0) << 7) | (mlen_bucket << 5) | ((cm->prev[0] >> 6) << 3) | bp;
        mixer_learn(&cm->mx6[mx6_ctx], str, bit, lr * 0.85f);
    ...
    sse_update(&cm->apm2, ((cm->prev[0] >> 4) << 8 | (cm->partial & 0x1F) << 3 | bp) & (SSE_CTXS-1), mp, bit);
