...
_mcx() {
    ...
    case "${prev}" in
        mcx)
            ...
            ;;
        compress)
            COMPREPLY=( $(compgen -W "-l -o -q -v -f -r -c -t --level -T --output --quiet --verbose --force --recursive --stdout --fast --default --best --delete --verify --threads --strategy --block-size --dry-run --estimate --level-scan --no-trials --filter --split --preserve-mtime --min-ratio --atomic --keep-broken --exclude" -- "${cur}") $(compgen -f -- "${cur}") )
            return 0
            ;;
        ...
        pipe)
            ...
            ;;
        bench|compare)
            COMPREPLY=( $(compgen -W "-l --level --compare --csv --warmup --json --decode-only --iterations --size --memory --all-levels --ratio-only --sort --top --median --percentile -t -T --threads" -- "${cur}") $(compgen -f -- "${cur}") )
            return 0
            ;;
        diff)
