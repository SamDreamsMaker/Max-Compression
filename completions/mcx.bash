...
_mcx() {
    ...
    case "${prev}" in
        ...
        pipe)
            ...
            ;;
        bench|compare)
            COMPREPLY=( $(compgen -W "-l --level --compare --csv --warmup --warmup-iterations --json --decode-only --iterations --size --memory --all-levels --ratio-only --sort --top --median --percentile --histogram --format --brief --worst --filter --exclude --aggregate --no-header --repeat -t -T --threads" -- "${cur}") $(compgen -f -- "${cur}") )
            return 0
            ;;
        diff)
