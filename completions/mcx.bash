...
_mcx() {
    ...
    case "${prev}" in
        mcx)
            ...
            ;;
        compress)
            COMPREPLY=( $(compgen -W "-l -o -q -v -f -r -c -t --level -T --output --quiet --verbose --force --recursive --stdout --fast --default --best --delete --verify --threads --strategy --block-size --dry-run --estimate --level-scan --level-range --no-trials --fast-decode --filter --split --preserve-mtime --min-ratio --atomic --keep-broken --exclude --decompress-check --priority --adaptive-level --memory-limit" -- "${cur}") $(compgen -f -- "${cur}") )
            return 0
            ;;
        ...
        pipe)
            ...
            ;;
        bench|compare)
            COMPREPLY=( $(compgen -W "-l --level --compare --csv --warmup --warmup-iterations --cold --json --decode-only --iterations --size --memory --all-levels --ratio-only --sort --top --median --percentile --histogram --format --brief --worst --filter --exclude --aggregate --no-header --repeat --output --compare-self --delta --save-baseline -t -T --threads" -- "${cur}") $(compgen -f -- "${cur}") )
            return 0
            ;;
        diff)
