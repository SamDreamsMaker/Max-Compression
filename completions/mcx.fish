...
complete -c mcx -n '__fish_seen_subcommand_from compress' -l best      -d 'Alias for --level-scan (try all, pick best)'
...
complete -c mcx -n '__fish_seen_subcommand_from compress' -l adaptive-level -d 'Auto-select level based on file entropy'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l memory-limit -x -d 'Cap memory usage (e.g. 256M, 1G)'
...
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l suite -r -a 'quick standard full' -d 'Predefined benchmark suite'
