...
complete -c mcx -n '__fish_seen_subcommand_from compress' -l level-range -x -d 'Try a range of levels, pick best (e.g. L1-L6)'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l no-trials   -d 'Skip multi-strategy trial at L20+'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l fast-decode -d 'Prefer faster-decoding entropy coders'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l filter     -x -a 'delta nibble none' -d 'Force preprocessing filter'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l split      -x -d 'Split output into chunks (e.g. 10M)'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l preserve-mtime -d 'Set output mtime to match input'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l min-ratio -x -d 'Only write if ratio >= N'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l atomic -d 'Write to temp file, rename on success'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l keep-broken -d 'Keep partial output on error'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l exclude -x -d 'Skip files matching glob pattern (with -r)'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l decompress-check -d 'Decompress in-memory after compress and verify CRC'
complete -c mcx -n '__fish_seen_subcommand_from compress' -l priority -x -a 'speed ratio balanced' -d 'Optimization goal (speed/ratio/balanced)'
...
complete -c mcx -n '__fish_seen_subcommand_from decompress extract x d' -l preserve-mtime -d 'Set output mtime to match input'
...
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l json -d 'JSON output format'
...
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l sort -x -a 'ratio speed level' -d 'Sort output by ratio/speed/level'
...
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l brief    -d 'Compact one-line-per-level output'
...
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l aggregate -d 'Show aggregate totals for directory benchmarks'
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l no-header -d 'Suppress column headers in output'
complete -c mcx -n '__fish_seen_subcommand_from bench compare' -l repeat -x -d 'Run benchmark N times, show min/max/avg'
