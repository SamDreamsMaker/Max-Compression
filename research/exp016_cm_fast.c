...
static inline float mixer_mix(mixer_t *mx, float * __restrict__ s) {
    float sum = 0;
    const float *w = mx->w;
    /* Unrolled by 4 for 29 models: 7 iterations + 1 remainder */
    int i = 0;
    for (; i + 3 < N_MODELS; i += 4)
...
static void cm_init(cm_t *cm, const uint8_t *data) {
    ...
    smap_init(&cm->o2, 1<<20);
    smap_init(&cm->o3, 1<<22);
    smap_init(&cm->o4, 1<<23);
    smap_init(&cm->o5, 1<<24);
    smap_init(&cm->o6, 1<<24);
    smap_init(&cm->o7, 1<<24);
    smap_init(&cm->word, 1<<22);
    smap_init(&cm->sparse13, 1<<20);
...
/* Predict: compute prob and cache state for update. Returns final prob + stores mp */
static uint16_t cm_predict(cm_t *cm, uint32_t pos, int bp,
                           float *str, uint32_t *ctx_out, uint16_t *mp_out) {
    cm_contexts(cm, pos, bp, ctx_out);
    
    smap_t *models[] = { &cm->o0, &cm->o1, &cm->o2, &cm->o3, &cm->o4,
        &cm->o5, &cm->o6, &cm->o7, &cm->word, &cm->sparse13,
        &cm->sparse14, &cm->sparse24, &cm->charclass, &cm->nibble,
        &cm->indirect, &cm->o2_word, &cm->gap15, &cm->delta,
        &cm->o1_nibble, &cm->o8, &cm->word2, &cm->sparse024,
        &cm->word_cc, &cm->o1_cc, &cm->word_len, &cm->prevword_byte,
        &cm->upper2, &cm->o4_cc };
    
    for (int i = 0; i < N_MODELS - 1; i++) {
        uint16_t p = smap_get(models[i], ctx_out[i]);
        str[i] = (p == PROB_HALF) ? 0.0f : stretch((float)p / PROB_MAX);
    }
    uint16_t mp_pred = match_predict(&cm->match, pos, bp);
    str[28] = (mp_pred == PROB_HALF) ? 0.0f : stretch((float)mp_pred / PROB_MAX);
    ...
    int mx4c = (((cm->prev[0] >> 4) << 5) | ((cm->prev[1] >> 4) << 1) | bp/4) & 1023;
    float m4 = mixer_mix(&cm->mx4[mx4c], str);
    int mx5c = (cm->word_hash ^ (cm->word_hash >> 8)) & 255;
    float m5 = mixer_mix(&cm->mx5[mx5c], str);
    float mixed = (m1 + m2 + m3 + m4 + m5) * 0.2f;
...
/* Update: use cached ctx/str from predict — no recomputation */
static void cm_update(cm_t *cm, int bp, int bit,
                      float *str, uint32_t *ctx, uint16_t mp) {
    smap_t *models[] = { &cm->o0, &cm->o1, &cm->o2, &cm->o3, &cm->o4,
        &cm->o5, &cm->o6, &cm->o7, &cm->word, &cm->sparse13,
        &cm->sparse14, &cm->sparse24, &cm->charclass, &cm->nibble,
        &cm->indirect, &cm->o2_word, &cm->gap15, &cm->delta,
    ...
    for (int i = 0; i < N_MODELS - 1; i++)
        smap_update(models[i], ctx[i], bit);
    
    float lr = 0.05f / (1.0f + (float)cm->total_bits / 20000.0f);
    if (lr < 0.003f) lr = 0.003f;
    ...
    int mx4c = (((cm->prev[0] >> 4) << 5) | ((cm->prev[1] >> 4) << 1) | bp/4) & 1023;
    mixer_learn(&cm->mx4[mx4c], str, bit, lr);
    int mx5c = (cm->word_hash ^ (cm->word_hash >> 8)) & 255;
    mixer_learn(&cm->mx5[mx5c], str, bit, lr);
    cm->total_bits++;
    
    int sse_ctx = ((cm->prev[0] >> 4) << 5 | (cm->partial & 0xF) << 1 | (bp >> 2)) & (SSE_CTXS-1);
    sse_update(&cm->sse, sse_ctx, mp, bit);
...
static size_t cm_compress(uint8_t *dst, size_t cap,
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        for (int bit = 7; bit >= 0; bit--) {
            ...
            uint16_t prob = cm_predict(&cm, (uint32_t)i, bp, str, ctx_cache, &mp);
            rcenc_bit(&rc, prob, b);
            cm_update(&cm, bp, b, str, ctx_cache, mp);
        }
