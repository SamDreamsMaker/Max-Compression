...
typedef struct {
    ...
    sse_t apm3;
    sse_t apm_par; /* parallel APM with different context */ /* fourth-stage APM — deepest in chain */
    mixer_t mx1[4096], mx2[128], mx3[8], mx4[1024], mx5[512], mx6[256], mx7[128];
    mixer_t mx8[512]; /* word_length bucket × bp */
    float lr;
    uint8_t prev[14];
    uint32_t word_hash;
...
static void cm_init(cm_t *cm, const uint8_t *data, size_t data_size) {
    ...
    smap_init(&cm->punctmod, 1 << hi_log); cm->punctmod.rate_n = 550;
    cm->punct_history = 0;
    smap_init(&cm->casemod, 1 << hi_log); cm->casemod.rate_n = 550;
    cm->case_history = 0;
    smap_init(&cm->sylmod, 1 << hi_log); cm->sylmod.rate_n = 550;
    cm->syl_count = 0;
    cm->last_was_vowel = 0;
    smap_init(&cm->vcmod2, 1 << hi_log); cm->vcmod2.rate_n = 550;
    smap_init(&cm->vcmod, 1 << hi_log);
    cm->vc_history = 0;
    smap_init(&cm->wposmod, 1 << hi_log); cm->wposmod.rate_n = 550;
    cm->word_pos = 0;
...
static void cm_free(cm_t *cm) {
    ...
    smap_free(&cm->o4ind); smap_free(&cm->o5ind); smap_free(&cm->o6ind); smap_free(&cm->o3ind2); if (cm->ictx8) free(cm->ictx8); if (cm->ictx7) free(cm->ictx7); if (cm->ictx6) free(cm->ictx6);
...
static void cm_update(cm_t *cm, uint32_t pos, int bp, int bit,
        ...
        smap_update(&cm->wposmod, wpos_ctx, bit);
    {
        ...
        smap_update(&cm->sylmod, syl_ctx, bit);
        {
            ...
            { uint32_t o3h2 = h32(((uint32_t)cm->prev[2]<<16)|((uint32_t)cm->prev[1]<<8)|cm->prev[0]) & (cm->ictx8_size - 1);
              smap_update(&cm->o3ind2, h32(((uint32_t)cm->ictx8[o3h2] << 11) | ((uint32_t)char_class(cm->prev[0]) << 8) | cm->partial), bit); }
            if (cm->match.active && cm->match.mpos < pos) {
        int ml = cm->match.mlen > 64 ? 64 : (int)cm->match.mlen;
        int ml_b = ml < 4 ? ml : ml < 8 ? 4 : ml < 16 ? 5 : ml < 32 ? 6 : 7;
        int pred_bit = (cm->match.data[cm->match.mpos] >> (7 - bp)) & 1;
